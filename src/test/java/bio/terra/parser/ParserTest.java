...
public class ParserTest {
  ...
  public void testSwitches() throws Exception {
    ...
    // parse everything
    String[] testString =
        new String[] {"test", "-a", "--rrr", "grrr", "-x", "xarg", "thereqarg", "theoptarg"};
    ParsedResult result = parser.parse(testString);
            ...
            new Command()
                ...
                .addOption(
                    new Option()
                        ...
                        .optional(false)
                        .help("rrr"))
                .addOption(new Option().shortName("x").hasArgument(true).optional(true).help("xxx"))
                .addArgument(new Argument().name("reqarg").optional(false).help("reqarg"))
                .addArgument(new Argument().name("optarg").optional(true).help("optarg")))
        .addCommand(
            new Command()
                .primaryNames(new String[] {"test2", "again"})
                .commandId(3)
                .alternateNames(new String[] {"testtube"})
                .addArgument(new Argument().name("myarg").optional(true)));
  }
}
